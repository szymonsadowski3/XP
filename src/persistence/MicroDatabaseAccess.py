...
class MicroDatabaseAccess:
    ...
    def get_user_by_id(self, identifier):
        filtered_users = [
            user for user in self.users if user.user_id == identifier]

        return filtered_users[0] if (len(filtered_users) > 0) else None

    def remove_user_by_username(self, username_of_user_to_remove):
        self.users = [
            user for user in self.users if user.username != username_of_user_to_remove]

    def remove_user_by_id(self, id_of_user_to_remove):
        self.users = [
            user for user in self.users if user.user_id != id_of_user_to_remove]
